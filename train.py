...
def trainer(args):
    ...
    backTranslator = BackTranslator(word_embedding, style_embedding, args.BackTranslator_gru_hidden_dim,
                                    args.BackTranslator_max_length)
    ...
    if torch.cuda.is_available():
        ...
        backTranslator = backTranslator.cuda()
    ...
    optimizer_tsl = optim.Adam(backTranslator.parameters(), lr=args.lr_translator)
    ...
    for i in tqdm.tqdm(range(args.maximum_iter), total=args.maximum_iter):
        ...
        if (i+1) % args.n_critic == 0:
            ...
            pred_cap_pair_loss = masked_cross_entropy(pred_cap[:, 1:, :].contiguous(),
                                                       ...
                                                       coco_cap_len - 1)
            pred_cap_pair_loss.backward(retain_graph=True)

            # neighbour loss
            if args.neighbor_loss_weight > 0:
                neighbor_loss = get_neighbor_loss(pred_cap, coco_cap_len) * args.neighbor_loss_weight
                neighbor_loss.backward(retain_graph=True)
            ...
            for style_id in range(args.style_num):
                ...
                pred_cap_style_loss.backward(retain_graph=True)
                pred_cap_style_loss_all.append(pred_cap_style_loss.item())

                # neighbour loss
                if args.neighbor_loss_weight > 0:
                    neighbor_loss = get_neighbor_loss(pred_cap_style, cap_len) * args.neighbor_loss_weight
                    neighbor_loss.backward(retain_graph=True)
        ...
        if (i+1) % args.n_critic == 0:
            ...
            if args.style_loss_weight > 0:
                # get factual cap encoding
                cap_emb = word_embedding(coco_cap)
                style_code_factual = torch.zeros(coco_cap.shape[0]).long()
                if torch.cuda.is_available():
                    style_code_factual = style_code_factual.cuda()
                _, encoder_hidden_tsl_factual = backTranslator(cap_emb, style_code_factual, encoder_only=True)
                encoder_hidden_tsl_factual = encoder_hidden_tsl_factual.detach()
                for style_id in range(1,args.style_num):  # skip factual
                    imageEncoder.zero_grad()
                    captionGenerator.zero_grad()
                    discriminator.zero_grad()
                    styleClassifier.zero_grad()
                    backTranslator.zero_grad()
                    style_code = torch.full((img_emb.shape[0],), style_id, dtype=torch.int).long()
                    if torch.cuda.is_available():
                        style_code = style_code.cuda()
                    _, encoder_hidden_tsl = backTranslator(cap_emb, style_code, encoder_only=True)
                    style_loss = get_style_loss(encoder_hidden_tsl_factual, encoder_hidden_tsl) * args.style_loss_weight
            ...
            for style_id in range(args.style_num):
                ...
                # compute the adversarial loss for caption generator
                # handling variable lengths
                if args.pack_lstm_input:
                ...
                # compute the back translation loss
                style_code_factual = torch.zeros(cap_emb_fake.shape[0]).long()  # 0 for fact
                if torch.cuda.is_available():
                    style_code_factual = style_code_factual.cuda()
                if use_guided_translation:
                    pred_tsl, encoder_hidden_tsl = backTranslator(cap_emb_fake, style_code_factual, guided_caption=coco_cap)
                else:
                    pred_tsl, encoder_hidden_tsl = backTranslator(cap_emb_fake, style_code_factual, guided_caption=None)
                    # todo: need to solve the length mismatch between pred and coco cap if no guided caption
                tsl_loss = masked_cross_entropy(pred_tsl, coco_cap, coco_cap_len)
...
def train():
    ...
    parser.add_argument('--neighbor_loss_weight', type=float, default=0.0,
                        help='neighbor loss weight')
    parser.add_argument('--style_loss_weight', type=float, default=0.0,
                        help='style hidden loss weight')

    parser.add_argument('--max_eval_number', type=int, default=1000,
                        help='maximum data for evaluation ')

    parser.add_argument('--use_GAN_adv', default='True', type=str2bool,
                        help='whether to GAN form adversarial loss')
